...
export default async function verifyAccessToken(req: NextRequest): Promise<NextResponse> {
  try {
    const accessToken =
...
export function middleware(req: NextRequest): NextResponse | Promise<NextResponse> | void {
  const path = req.nextUrl.pathname;
  const accessToken =
    req.cookies.get("accessToken")?.value ||
    req.headers.get("Authorization")?.replace("Bearer ", "");

  const isPublicPath = path === "/users/login" || path === "/users/signup" || path === "/";
  ...
  if (path.includes("/api/invoice") || path.includes("/api/users/getLoggedInUser")) {
    try {
